...
class HourTests : TestDate() {
    ...
    fun hour_floor_date_returns_hours() {
        ...
        time.floor(date(2010, 12, 31, 23, 59)) shouldBe date(2010, 12, 31, 23, 0)
        time.floor(date(2011, 1, 1, 0, 0)) shouldBe date(2011, 1, 1, 0, 0)
        time.floor(date(2011, 1, 1, 0, 1)) shouldBe date(2011, 1, 1, 0, 0)
    }
    ...
    fun hour_ceil_returns_hours() {
        ...
        time.ceil(date(2010, 12, 31, 23, 59)) shouldBe date(2011, 1, 1, 0, 0)
        time.ceil(date(2011, 1, 1, 0, 0)) shouldBe date(2011, 1, 1, 0, 0)
        time.ceil(date(2011, 1, 1, 0, 1)) shouldBe date(2011, 1, 1, 1, 0)
    }
    ...
    fun hour_offset_allows_negative_positive_zero_step() {
        ...
        time.offset(date(2010, 12, 31, 12, 0), -1) shouldBe date(2010, 12, 31, 11, 0)
        time.offset(date(2011, 1, 1, 1, 0), -2) shouldBe date(2010, 12, 31, 23, 0)
        time.offset(date(2011, 1, 1, 0, 0), -1) shouldBe date(2010, 12, 31, 23, 0)

        time.offset(date(2010, 12, 31, 11, 0), 1) shouldBe date(2010, 12, 31, 12, 0)
        time.offset(date(2010, 12, 31, 23, 0), 2) shouldBe date(2011, 1, 1, 1, 0)
        time.offset(date(2010, 12, 31, 23, 0), 1) shouldBe date(2011, 1, 1, 0, 0)
    ...
    fun hour_range_start_stop_cans_kip_hours() {
        ...
        val range = time.range(date(2011, 2, 1, 1, 0), date(2011, 2, 1, 13, 0), 3)
    ...
    fun hour_range_start_stop_returns_hours_between_start_inclusive_and_stop_exclusive() {
        val time = timeHour
        val result = listOf(
                date(2010, 12, 31, 23, 0),
                date(2011, 1, 1, 0, 0),
                date(2011, 1, 1, 1, 0)
        )

        val range = time.range(date(2010, 12, 31, 23, 0), date(2011, 1, 1, 2, 0))
        range.forEachIndexed { index, r ->
            r shouldBe result[index]
        }
    }
