...
abstract class ComposedProjection() :
    ...
    override fun fullCycleStream(stream: Stream): Stream = MultiplexStream(allProjections.map { it.stream(stream) })


    override fun fitExtent(extent: Extent, geo: GeoJsonObject): Projection =
        io.data2viz.geo.fitExtent(mainProjection, extent, geo)


    override fun fitWidth(width: Double, geo: GeoJsonObject): Projection =
        io.data2viz.geo.fitWidth(mainProjection, width, geo)


    override fun fitHeight(height: Double, geo: GeoJsonObject): Projection =
        io.data2viz.geo.fitHeight(mainProjection, height, geo)


    override fun fitSize(width: Double, height: Double, geo: GeoJsonObject): Projection =
        io.data2viz.geo.fitSize(mainProjection, width, height, geo)
