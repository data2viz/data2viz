...
class HSL(var _h: Angle = Angle(0.0), var _s: Float = 1f, var _l: Float = 1f, var _alpha: Float = 1.0f) {
    ...
    fun rgb(): Color {
        ...
        // see MOZILLA HSL specification
        val hueAngle = if (h.deg >= 0) h.deg % 360 else h.deg % 360 + 360
        ...
        if (s == 0f) {                          // achromatic
            ...
            val hue2rgb = fun(p: Float, q: Float, hueInDegree: Double): Float {
                val hd = if (hueInDegree >= 0) hueInDegree % 360 else hueInDegree % 360 + 360
                if (hd < 60) return (p + (q - p) * (hd / 60.0)).toFloat()
                if (hd < 180) return q
                if (hd < 240) return (p + (q - p) * ((240 - hd) / 60.0)).toFloat()
                return p
            }

            val q = if (l < 0.5f) l * (1 + s) else l + s - l * s
            val p = 2 * l - q;
            rgb.r = Math.round(hue2rgb(p, q, hueAngle + 120.0) * 255)
            rgb.g = Math.round(hue2rgb(p, q, hueAngle) * 255)
            rgb.b = Math.round(hue2rgb(p, q, hueAngle - 120.0) * 255)
        }
        return rgb
    }
