...
fun GroupNode.buildSankey() {
    ...
    sankey.links.forEach { link ->
        val gradient = Colors.Gradient.linear(
            Point(
                if (sankeyHorizontal) link.source.x1 else link.source.y1,
                if (sankeyHorizontal) link.source.y1 else link.source.x1
            ),
            Point(
                if (sankeyHorizontal) link.target.x0 else link.target.y0,
                if (sankeyHorizontal) link.target.y0 else link.target.x0
            )
        )
