...
fun main(args: Array<String>) {
    js("console.time('delaunay')")
    var delaunay = Delaunator(points)
    js("console.timeEnd('delaunay')")
    ...
    val padding = 5
    val w = 1024
    val h =( (w - 2 * padding) * (maxY - minY) / (maxX - minX) + 2 * padding).roundToInt()
    ...
    canvas.onmousemove = { event ->
        ...
        points.asDynamic().push(
            arrayOf((e.layerX - padding) / ratio + minX,
            (e.layerY - padding) / ratio + minY))
        updateDelaunay()
    }


    fun getX(i:Int) = padding + ratio * (points[i][0] - minX)
    fun getY(i:Int) = padding + ratio * (points[i][1] - minY)
    ...
    fun draw() {
        ...
        ctx.beginPath()
        for (i in 0 until triangles.length step 3) {
            val p0 = triangles[i]
        ...
        ctx.beginPath()
        for (i  in 0 until points.size) {
            ctx.rect(getX(i) - 1.5, getY(i) - 1.5, 3.0, 3.0)
        }
        ctx.fill()
    }
