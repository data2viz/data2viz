...
class DayTests : TestDate() {
    ...
    fun day_floor_date_returns_days() {
        timeDay.floor(zoneUTC, dateUTC(2010, 12, 31, 23, 0)) shouldBe dateUTC(2010, 12, 31, 0, 0)
        timeDay.floor(zoneUTC, dateUTC(2011, 1, 1, 0, 0)) shouldBe dateUTC(2011, 1, 1, 0, 0)
        timeDay.floor(zoneUTC, dateUTC(2011, 1, 1, 1, 0)) shouldBe dateUTC(2011, 1, 1, 0, 0)
    }

    @Test
    fun day_handles_years_in_first_century() {
        timeDay.floor(zoneUTC, dateUTC(11, 10, 6, 7, 0)) shouldBe dateUTC(11, 10, 6, 0, 0)
    }
    ...
    fun day_offset_date_step_does_not_round_the_passed_date() {
        ...
        timeDay.offset(zoneUTC, date1, 1) shouldBe date2

        val date3 = dateUTC(2010, 12, 29, 23, 59, 59, 456)
        timeDay.offset(zoneUTC, date1, -2) shouldBe date3
    }
    ...
    fun day_count_start_end_counts_days_after_start_exclusive_and_before_end_inclusive() {
        timeDay.count(zoneLocal, dateLocal(2011, 1, 1, 0), dateLocal(2011, 5, 9, 0, 0)) shouldBe 128
...
//        timeDay.count(zoneLocal, dateLocal(2010, 12, 31, 23), dateLocal(2011, 5, 9, 0, 0)) shouldBe 129
        timeDay.count(zoneLocal, dateLocal(2011, 1, 1, 0), dateLocal(2011, 5, 8, 23, 0)) shouldBe 127
        timeDay.count(zoneLocal, dateLocal(2011, 1, 1, 0), dateLocal(2011, 5, 9, 1, 0)) shouldBe 128
    }

    @Test
    fun day_count_start_end_observes_daylight_saving() {
        timeDay.count(zoneUTC, dateUTC(2011, 1, 1, 0, 0), dateUTC(2011, 3, 13, 1, 0)) shouldBe 71
        timeDay.count(zoneUTC, dateUTC(2011, 1, 1, 0, 0), dateUTC(2011, 3, 13, 3, 0)) shouldBe 71
        timeDay.count(zoneUTC, dateUTC(2011, 1, 1, 0, 0), dateUTC(2011, 3, 13, 4, 0)) shouldBe 71
        timeDay.count(zoneUTC, dateUTC(2011, 1, 1, 0, 0), dateUTC(2011, 11, 6, 0, 0)) shouldBe 309
        timeDay.count(zoneUTC, dateUTC(2011, 1, 1, 0, 0), dateUTC(2011, 11, 6, 1, 0)) shouldBe 309
        timeDay.count(zoneUTC, dateUTC(2011, 1, 1, 0, 0), dateUTC(2011, 11, 6, 2, 0)) shouldBe 309
    }
    ...
    fun day_floor_handles_DST() {
        timeDay.floor(zoneLocal, dateUTC(2011, 3, 13, 7, 0)) shouldBe dateLocal(2011, 3, 12, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 3, 13, 8, 0)) shouldBe dateLocal(2011, 3, 13, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 3, 13, 9, 0)) shouldBe dateLocal(2011, 3, 13, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 3, 13, 10, 0)) shouldBe dateLocal(2011, 3, 13, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 11, 6, 7, 0)) shouldBe dateLocal(2011, 11, 6, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 11, 6, 8, 0)) shouldBe dateLocal(2011, 11, 6, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 11, 6, 9, 0)) shouldBe dateLocal(2011, 11, 6, 0, 0)
        timeDay.floor(zoneLocal, dateUTC(2011, 11, 6, 10, 0)) shouldBe dateLocal(2011, 11, 6, 0, 0)
    }

    // TODO fix the commented tests
    @Test
    fun day_round_handles_DST() {
        with(timeDay) { zoneLocal.round(dateUTC(2011, 3, 13, 7, 0)) } shouldBe dateLocal(2011, 3, 13, 0, 0)
        with(timeDay) { zoneLocal.round(dateUTC(2011, 3, 13, 8, 0)) } shouldBe dateLocal(2011, 3, 13, 0, 0)
        with(timeDay) { zoneLocal.round(dateUTC(2011, 3, 13, 9, 0)) } shouldBe dateLocal(2011, 3, 13, 0, 0)
        with(timeDay) { zoneLocal.round(dateUTC(2011, 3, 13, 20, 0)) } shouldBe dateLocal(2011, 3, 14, 0, 0)
        with(timeDay) { zoneLocal.round(dateUTC(2011, 11, 6, 7, 0)) } shouldBe dateLocal(2011, 11, 6, 0, 0)
        with(timeDay) { zoneLocal.round(dateUTC(2011, 11, 6, 8, 0)) } shouldBe dateLocal(2011, 11, 6, 0, 0)
        with(timeDay) { zoneLocal.round(dateUTC(2011, 11, 6, 9, 0)) } shouldBe dateLocal(2011, 11, 6, 0, 0)
//        with(timeDay) { zoneLocal.round(dateUTC(2011, 11, 6, 20, 0)) } shouldBe dateLocal(2011, 11, 7, 0, 0)
    }

    // TODO fix the commented tests
    @Test
    fun day_ceil_handles_DST() {
        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 3, 13, 7, 0)) } shouldBe dateLocal(2011, 3, 13, 0, 0)
        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 3, 13, 8, 0)) } shouldBe dateLocal(2011, 3, 13, 0, 0)
        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 3, 13, 9, 0)) } shouldBe dateLocal(2011, 3, 14, 0, 0)
        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 3, 13, 20, 0)) } shouldBe dateLocal(2011, 3, 14, 0, 0)

        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 11, 6, 7, 0)) } shouldBe dateLocal(2011, 11, 6, 0, 0)
//        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 11, 6, 8, 0)) } shouldBe dateLocal(2011, 11, 7, 0, 0)
//        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 11, 6, 9, 0)) } shouldBe dateLocal(2011, 11, 7, 0, 0)
//        with(timeDay) { zoneLocal.ceil(dateUTC(2011, 11, 6, 10, 0)) } shouldBe dateLocal(2011, 11, 7, 0, 0)
    }

    @Test
    fun day_ceil_handles_midnight_leap_years() {
        with(timeDay) { zoneLocal.ceil(dateUTC(2012, 3, 1, 0, 0)) } shouldBe dateLocal(2012, 3, 1, 0, 0)
    }
